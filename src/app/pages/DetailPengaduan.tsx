...
import { ArrowLeft, Calendar, MapPin, MessageSquare, Send, Tag, User } from "lucide-react";
import { COMPLAINT_STATUSES, STATUS_COLORS } from "../constants/complaint";
...
export default function DetailPengaduan() {
  ...
  if (!complaint) {
    return (
      <div className="space-y-4">
        <button onClick={() => navigate("/pengaduan")} className="flex items-center gap-2 text-blue-600 hover:text-blue-700">
  ...
  const getUserName = (userId: string) => (userId === "user-002" ? "John Doe" : "Anonymous");

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate("/pengaduan")} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
        ...
        {isAdmin && (
          <select
            ...
            className={`px-4 py-2.5 border-0 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none font-medium cursor-pointer ${getStatusColor(complaint.status)}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <h2 className="text-xl font-bold text-gray-800 mb-4">{complaint.title}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {complaint.customCategoryDescription && (
                    <p className="text-sm text-gray-600 mt-1">{complaint.customCategoryDescription}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{complaint.subDistrict}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <Calendar className="w-5 h-5 text-gray-400 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-sm text-gray-600">Tanggal Ditangani</p>
                  <p className="font-medium text-gray-800">
                    {complaint.handledAt
                      ? new Date(complaint.handledAt).toLocaleDateString("id-ID", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                          hour: "2-digit",
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between mb-4">
              ...
              {isAdmin && (
                <button onClick={() => setShowReplyModal(true)} className="flex items-center gap-2 text-blue-600 hover:text-blue-700 text-sm font-medium">
            ...
            {complaint.adminResponse.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <p>Belum ada balasan dari admin</p>
              </div>
            ) : (
              <div className="space-y-4">
                {complaint.adminResponse.map((response) => (
                  <div key={response.id} className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <p className="font-medium text-gray-800">{response.adminName}</p>
                    ...
                    <p className="text-gray-700 text-sm leading-relaxed">{response.message}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-medium text-gray-800">{getUserName(complaint.userId)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            ...
            <div className="text-center py-4">
              <span className={`inline-block px-4 py-2 font-medium rounded-full text-lg ${getStatusColor(complaint.status)}`}>
                {complaint.status}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <h3 className="font-semibold text-gray-800 mb-4">Status Balasan Admin</h3>
            <div className="text-center py-4">
              <span
